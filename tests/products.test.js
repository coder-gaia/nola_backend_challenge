...
describe("Products Controller Endpoints", () => {
  ...
  describe("GET /api/analytics/top-products", () => {
    it("deve retornar os 10 produtos mais vendidos com sucesso", async () => {
      ...
      expect(res.body.data[0].total_sales).toBe(250);
      expect(res.body.data[0].total_revenue).toBeCloseTo(3750.5);
      expect(query).toHaveBeenCalledTimes(1);
    });

    it("deve incluir channel_id nos parâmetros quando enviado", async () => {
      query.mockResolvedValueOnce({ rows: [] });

      const res = await request(app).get(
        "/api/analytics/top-products?start=2025-01-01&end=2025-01-31&channel_id=2"
      );

      expect(res.status).toBe(200);
      expect(res.body.params.channel_id).toBe("2");
      expect(query).toHaveBeenCalledTimes(1);
    });
  });
